...
public class VXVMTDataPreparator {
    ...
    static public VXVMTData LoadData(IBroker broker) {
        ...
        if (dataVXV == null || dataVXV.adjCloses == null || dataVXV.dates == null || dataVXV.adjCloses.length == 0 || dataVXV.dates.length == 0) {
            logger.severe("Failed to load VXV");
            return null;
        }
        
        if (dataVXMT == null || dataVXMT.adjCloses == null || dataVXMT.dates == null || dataVXMT.adjCloses.length == 0 || dataVXMT.dates.length == 0) {
            logger.severe("Failed to load VXMT");
            return null;
        }
        ...
        if (!VXVMTChecker.CheckTickerData(dataVXV, "VXV")
                || VXVMTChecker.CheckTickerData(dataVXMT, "VXMT")) {
            return null;
        }
